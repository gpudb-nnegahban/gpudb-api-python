...
def example_get_trigger_info():
    ...
    gaiadb.register_trigger_range( trigger_id1, [set_id1, set_id2], "x", 0, 3 )
    gaiadb.register_trigger_range( trigger_id2, [set_id1], "x", -0.05, 30 )
    gaiadb.register_trigger_range( trigger_id3, [set_id2], "x", -1, 2 )

    # Add some objects to the two sets
    gaiadb.bulk_add_point( set_id1,
                              [-1, 0, 0.2, 1.2, 2, 3, 3.4, -1.2, -0.2, 0.42],
                              [-1, 0, 0.2, 1.2, 2, 3, 3.4, -1.2, -0.2, 0.42] )
    gaiadb.bulk_add_point( set_id2, range(-5, 10), range(-5, 10) )

    # Retrieve trigger info
    print
    print "Info regarding trigger 1: ", gaiadb.get_trigger_info( [trigger_id1] )

    print "Formatted output for all trigger information:"
    trigger_info_map = gaiadb.get_trigger_info( [] )["trigger_map"]
    ...
    # Clear the trigger
    gaiadb.clear_trigger( trigger_id1 )
    gaiadb.clear_trigger( trigger_id2 )
    gaiadb.clear_trigger( trigger_id3 )
